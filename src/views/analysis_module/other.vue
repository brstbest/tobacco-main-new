...
import { getOtherData, getAlertData, getBakeFigure } from '@/api/bake'
...
export default {
  ...
  data() {
    return {
      ...
      tabColumnsList: [
        ...
        [
          {key: 'location', label: '产区'},
          {key: 'sum_baking_weight_before', label: '累计烤前重量'},
          {key: 'sum_baking_weight_after', label: '累计烤后重量'},
  ...
  methods: {
    ...
    setDataTable_4() {
      let res = []
      let flag = true
      this.bakeData.forEach(data => {
        res.forEach(item => {
          if(item.location === data.location && item.parent_location === data.parent_location){
            item.sum_baking_weight_before = item.sum_baking_weight_before + data.baking_weight_before
            item.sum_baking_weight_after = item.sum_baking_weight_after + data.baking_weight_after
            item.avg_score = item.avg_score + data.score
        ...
        if(flag){
          let obj = {
            location: data.location,
            parent_location: data.parent_location,
            sum_baking_weight_before: data.baking_weight_before,
            sum_baking_weight_after: data.baking_weight_after,
      ...
      res.forEach(item => {
        item.avg_score = (item.avg_score / item.count).toFixed(2)
      })
      res.sort(function (x, y) {
        return y.avg_score - x.avg_score
      })
      this.tableData.push(res)
    },
    ...
    requestAlertData(rdata){
      ...
      getAlertData(rdata).then(res => {
        ...
        if(res.code === 200){
          ...
          this.setDataTable_2()
          this.setDataTable_3()
          this.requestBakeData(rdata)
        }
      }).catch(err => {
        this.$message.error(err)
      })
    },
    requestBakeData(rdata){
      getBakeFigure(rdata).then(res => {
        console.log('getBakeFigure', res)
        if(res.code === 200){
          this.bakeData = res.data
          this.setDataTable_4()
        }
      }).catch(err => {
        this.$message.error('aaasasasaa'+err)
      })
    }
  }
}
