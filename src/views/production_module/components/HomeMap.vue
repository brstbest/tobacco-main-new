    ...
    requestGetProvinceJSON(params) {
      ...
      getMapData(data).then(r => {
        ...
        hasDeviceData.children.forEach(p => {
          t[this.postcodeJSON[p.location]] = { 'total': p.total, 'abnormal': p.abnormal, 'children': p.children }
          this.position2postcode[this.postcodeJSON[p.location]] = p.location
          // t[String(postcodes[p.location])] = p['device']
          // this.position2postcode[postcodes[p.location]] = p.location
        })
        console.log('t', t)
        console.log('position2postcode', this.position2postcode)
    ...
    requestGetCityJSON(params) {
      ...
      this.deviceData.children.forEach(c => {
        c.children.forEach(child => {
          t[this.postcodeJSON[child.location]] = { 'total': child.total, 'abnormal': child.abnormal }
          this.position2postcode[this.postcodeJSON[child.location]] = child.location
        })
      })
      hasDeviceData = t
      // let t = {}
